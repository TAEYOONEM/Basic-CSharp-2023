...
namespace wf13_bookrentalshop
{
    public partial class FrmBooks: Form
    {
        ...
        private void BtnDel_Click(object sender, EventArgs e)
        {
            ...
            // FK 제약 조건으로 지울 수 없는 데이터인지 먼저 확인
            using (MySqlConnection conn = new MySqlConnection(Commons.ConnString))
            {
                ...
                MySqlParameter prmDivion = new MySqlParameter("@Division", TxtBookIdx.Text);
                chkCmd.Parameters.Add(prmDivion);
        ...
        private void SaveData()
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Commons.ConnString))
                {
                    ...
                    if (isNew == false) // update 할때는 bookIdx 파라미터 추가
                    {
